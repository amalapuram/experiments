...
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
@@@  uncomment line number 149 for WACB setup
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
'''
...
for name in names:
  ...
  data = np.load("../data/DIL_CBRS_NODUPLICATE/"+name1,allow_pickle=True) # loads the individual data for each class
  arrs[name] = data
...
for i,name in enumerate(names):
  ...
  #print(class_length)
  train_split = int(0.75*class_length)    # I used 0.70 because I want 70% of the dataset to be train data, change this according to preference
...
for task in task_order:
  for i,class_ in enumerate(task):
    ...
    #print("original class label is ",class_)
    #print("class encoding is:",class_encoding)
...
hidden_size = 100
model = Sequential()
model.add(Conv2D(14, kernel_size=(3, 3), activation='relu', input_shape=input_shape,use_bias=False))
#model.add(Dropout(0.2))
model.add(Flatten())
model.add(Dense(hidden_size, activation='relu',use_bias=False))
model.add(Dropout(0.5))
model.add(Dense(1, activation='sigmoid',use_bias=False))
m_weights=model.get_weights()
for i in range(len(m_weights)):
    m_weights[i]=(m_weights[i]).ravel()
m_weights=np.concatenate(m_weights).ravel()
param_weights=np.array(m_weights)
#print(param_weights.shape)



optimizer = tf.keras.optimizers.Adamax(lr=0.001)      #Optimizer to use while training
loss_fn = tf.keras.losses.BinaryCrossentropy()   
optimizer2 = tf.keras.optimizers.Adamax(lr=0.001)      #Optimizer to use while training
loss_fn2 = tf.keras.losses.BinaryCrossentropy()         #Loss function
train_acc_metric = tf.keras.metrics.BinaryAccuracy()     #Accuracy metric for training - this metric will be displayed every few epochs in training
val_acc_metric = tf.keras.metrics.BinaryAccuracy()       #Accuracy metric for validation dataset
print(model.summary())
...
for X,y,y_classname in tasks:
  ...
  for i in range(0, task_size, b):
    ...
    numbers = np.random.choice(range(0,memory_y_name.shape[0]), min(b,memory_y_name.shape[0]), replace=False, p=probas) 
    ...
    for ind,rand in enumerate(range(len(memory_X))): #loading the samples from CBRS Memory which will replay later
    ...
    for epoch in range(epochs):
        ...
        #    m_weights[i]=(m_weights[i]).ravel()
        #m_weights=np.concatenate(m_weights).ravel()
        #param_weights=np.vstack((param_weights,m_weights))
        ...
        for step, (stream, replay) in enumerate(zip(stream_dataset,replay_dataset)):
           ...
           # m_weights=np.concatenate(m_weights).ravel()
           # param_weights=np.vstack((param_weights,m_weights)) 
            #print("shape is",param_weights.shape)
            ...
            with tf.GradientTape() as tape:
                ...
                #for i in range(len(model.get_weights())):
                 #   print("layer:"+str(i)+"is",len(model.get_weights()[i].ravel()))

                #exit()
                temp_model.set_weights(model.get_weights())

                if mean_size < threshold_iter:
                    #temp_model=tf.keras.models.clone_model(model)
                    temp_model.set_weights(model.get_weights())
                #with tf.GradientTape() as temp_tape:
                    stream_logits = temp_model(x_stream_train, training=True)
                    stream_loss = loss_fn2(y_stream_train, stream_logits)
                    grads = tape.gradient(stream_loss, temp_model.trainable_weights)
                    optimizer2.apply_gradients(zip(grads, temp_model.trainable_weights))
                    m_weights=model.get_weights()
                    for i in range(len(m_weights)):
                        #print("layer:"+str(i)+"size is:",m_weights[i].shape)
                        m_weights[i]=(m_weights[i]).ravel()
                    ...
                    mean_size+=1
                    if (mean_size==threshold_iter):
                      param_mean=np.mean(param_weights,axis=0)
                  ...
                  for i in range(len(m_weights)):
                    m_weights[i]=(m_weights[i]).ravel()
                    if i==0:
                      split_idx.append(len(m_weights[i]))
                    else:
                      split_idx.append(len(m_weights[i])+split_idx[i-1]) 
                  del split_idx[-1]
                  m_weights=np.concatenate(m_weights).ravel()
                  param_mean=np.array(param_mean*0.5+0.5*m_weights)
                  #print("split idx",split_idx)
                  split_param_array=np.split(param_mean,split_idx,axis=0)
                  #print("paramter are:",split_param_array)
                  m_weights=temp_model.get_weights()
                  j=0
                  for i in range(len(m_weights)):
                    if len(temp_model.layers[i].get_weights())!=0:
                      #print("wigts of layers"+str(i)+"is",temp_model.layers[i].get_weights())
                      #print("requires shape is:",m_weights[j].shape)
                      updated_weight=np.array(np.reshape(split_param_array[j],m_weights[j].shape),dtype=float)
                      #print("current shape is:",updated_weight.shape)
                      #print(updated_weight)
                      temp_model.layers[i].set_weights([updated_weight])
                      j+=1
                  #exit()
                                
            #del tape
            MIR_Loss=(abs(temp_model.predict(x_replay_train.numpy()).ravel().round()-model.predict(x_replay_train.numpy()).ravel().round())).astype('int')
                #MIR_Loss=MIR_Loss.bool()
                #del tape
            total_interfered_samples+=np.count_nonzero(MIR_Loss == 1)
                #if temp > 0:
                 #   print("Numer of interfere samples is:",temp)
            x_replay_train=tf.boolean_mask(x_replay_train,MIR_Loss)
            y_replay_train=tf.boolean_mask(y_replay_train,MIR_Loss)
            x_replay_train=tf.cast(x_replay_train,dtype=tf.float32)
            y_replay_train=tf.cast(y_replay_train,dtype=tf.int64)

                #print(type(x_replay_train))
            x_stream_train=tf.concat([x_stream_train,x_replay_train],0)
            y_stream_train=tf.concat([y_stream_train,y_replay_train],0)
            MIR_time+=time.time()-mem_begin
            ...
            #del tape
            with tf.GradientTape() as temp_tape:
                stream_logits = model(x_stream_train, training=True)
                stream_loss = loss_fn(y_stream_train, stream_logits)
                loss_value = stream_loss
                grads = temp_tape.gradient(loss_value, model.trainable_weights)
...
print("Parameters weight size:",param_weights.shape)
np.save("param_weight_mem_"+str(memory_size)+"__mini_batch_"+str(batch_size)+"virtual_update",param_weights)
...
print("Total  MIR time is--- %s seconds ---" % (MIR_time))
print("Total no of interfered samples are:",total_interfered_samples)
print(X_test.shape)
