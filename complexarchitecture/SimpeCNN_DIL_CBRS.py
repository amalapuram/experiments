...
for name in names:
  ...
  data = np.load(name1,allow_pickle=True)
  arrs[name] = data
...
for i,name in enumerate(names):
  ...
  l = arr_x.shape[0]
  print(l)
  split1 = int(0.66*l)    # I used 0.70 because I want 70% of the dataset to be train data, change this according to preference
...
'''
name_to_id = {'DoS Hulk':1, 'PortScan':2, 'DDoS':3, 'FTP-Patator':4, 'DoS slowloris':5, 'DoS Slowhttptest':6, 'SSH-Patator':7, 'Bot':8, 'Web Attack � Brute Force':9, 'DoS GoldenEye':10, 'Web Attack � XSS':11, 'Infiltration':12, 'Web Attack � Sql Injection':13, 'Heartbleed':14,'Benign15':15,'Benign16':16,'Benign17':17,'Benign18':18,'Benign19':19}
...
task_order = [('Heartbleed','SSH-Patator', 'DoS Hulk','Benign15' ), ('Infiltration', 'Bot', 'Web Attack � Brute Force','Benign16' ), ('Benign17','DoS Slowhttptest','FTP-Patator'), ('Web Attack � Sql Injection', 'Web Attack � XSS', 'DoS slowloris','Benign18'), ('PortScan','DoS GoldenEye', 'DDoS','Benign19')]
'''
...
optimizer = tf.keras.optimizers.Adam(lr=0.001)      #Optimizer to use while training
...
for X,y,y_name in tasks:
  ...
  np.save("./"+ model_name,train_losses)
  train_losses = []
  
  '''
  model_name = 'cbrs_binary2_benign1_adamax_val_loss' + str(loop)
  np.save("./"+ model_name,val_losses)
  val_losses = []
  '''
