...
# log to Tensorboard
tb_logger = TensorboardLogger(f"./tb_data/{cur_time}-simpleCNN_Domain/")

# log to text file
text_logger = TextLogger(open(f"./logs/{cur_time}-simpleCNN_Domain.txt", "w+"))
...
for task_number, experience in enumerate(generic_scenario.train_stream):
    ...
    torch.save(
        model.state_dict(),
        "./weights/simpleCNN_Domain/After_training_Task_{}".format(task_number + 1),
    )
    print("Model saved!")
